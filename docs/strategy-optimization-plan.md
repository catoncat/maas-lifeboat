...
这个文档记录当前实验结果和下一轮研究方向。目标不是继续证明 provider 不稳定；这个已经足够清楚。目标是把 gateway 的策略继续往“更高最终成功率、更低后端压力、更低用户等待时间”推进。
...
| provider 的 busy 信号 | HTTP `503` + provider code `10310` |
...
| 问题 | 当前答案 | 证据 |
| --- | --- | --- |
| 失败外观 | provider 用 `503/10310` 暴露 busy/容量/限流类状态 | 所有失败样本几乎都是同一错误 |
| 是否本地 gateway bug | 不像 | probe 直接打后端也复现同一错误 |
...
| 观察 | 结果 | 策略含义 |
| --- | --- | --- |
| paired 窗口单接口 | OpenAI only 9/20；Anthropic only 4/20 | 单接口会吃到大量短窗口 busy |
...
当前最强信号不是“换接口就好了”，而是：**不要让同一个账号同时跑多个长 generation；busy 后不要立即打下一枪；转换只作为弱去相关 fallback。**
...
| --- | --- | --- |
| 账号级限制 vs provider 全局拥塞 | 只有一个账号，且项目不走多账号路线 | 只测“本账号经验可用边界”，不把不可验证的 provider 内部机制写成定论 |
| 模型池限制 vs route/IP | 当前只有一个主模型，route 样本非随机 | 暂不作为主力；只有 queue/cooldown dogfood 后仍明显不够，再做小块随机化 |
| 精确 RPM/TPM | provider 不返回标准限流 envelope，而是统一 `503/10310` | 只记录经验边界，不把拟合阈值写成稳定 rate limit |
| PI 卡顿来自哪里 | gateway 只有服务端 attempt/stream 时间，缺少 PI 客户端 chunk 时间戳 | 增加 client-visible chunk timing 或 PI 侧日志 |
...
| client retry 协同 | final `503/529` 后等待短 jitter 再 retry 可能比继续打满 attempts 更好 | 依赖客户端行为 | all-busy 时返回 `MAAS_ALL_BUSY_RETRY_AFTER_S`，测 PI 是否按预期 retry |
